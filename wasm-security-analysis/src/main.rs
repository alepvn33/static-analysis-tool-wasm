...
//            write!(f, "{}", self.type_())
    ...
    println!(
        ...
        cfi_eq_classes.iter().map(|(_class, size)| size).min().unwrap_or(&0), // Use 0 as default if no classes present
        cfi_eq_classes.iter().map(|(_class, size)| size).max().unwrap_or(&0), // Use 0 as default if no classes present
        avg(&cfi_eq_classes.iter().map(|(_class, size)| *size).collect::<Vec<usize>>()),
        median(&cfi_eq_classes.iter().map(|(_class, size)| *size).collect::<Vec<usize>>()),
...
fn median(numbers: &Vec<usize>) -> usize {
    ...
    if numbers.is_empty() {
        0 // Return 0 as the median if the vector is empty
    } else {
        numbers[mid]
    }
}
